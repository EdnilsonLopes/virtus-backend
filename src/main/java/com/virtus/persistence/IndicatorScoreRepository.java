...
public interface IndicatorScoreRepository extends BaseRepository<IndicatorScore> {
    ...
    List<IndicatorScore> findByCnpbAndReferenceDate(@Param("cnpb") String cnpb, @Param("referenceDate") String referenceDate);
    ...
    // Paginated + filter
    @Query(value = "SELECT * FROM virtus.notas_indicadores WHERE " +
                   "(:filter IS NULL OR LOWER(sigla_indicador) LIKE %:filter% OR LOWER(tx_componente) LIKE %:filter%)",
           countQuery = "SELECT COUNT(*) FROM virtus.notas_indicadores WHERE " +
                        "(:filter IS NULL OR LOWER(sigla_indicador) LIKE %:filter% OR LOWER(tx_componente) LIKE %:filter%)",
           nativeQuery = true)
