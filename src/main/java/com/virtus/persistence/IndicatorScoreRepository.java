...
public interface IndicatorScoreRepository extends BaseRepository<IndicatorScore> {
        ...
        @Query(value = "SELECT * FROM virtus.notas_indicadores " +
                        "WHERE CAST(cnpb AS VARCHAR) LIKE '%' + :filter + '%' " +
                        "   OR CAST(nota AS VARCHAR) LIKE '%' + :filter + '%' " +
                        "   OR LOWER(sigla_indicador) LIKE '%' + LOWER(:filter) + '%' " +
                        "   OR data_referencia LIKE '%' + :filter + '%' " +
                        "ORDER BY data_referencia DESC",
                        countQuery = "SELECT COUNT(*) FROM virtus.notas_indicadores " +
                                        "WHERE CAST(cnpb AS VARCHAR) LIKE '%' + :filter + '%' " +
                                        "   OR CAST(nota AS VARCHAR) LIKE '%' + :filter + '%' " +
                                        "   OR LOWER(sigla_indicador) LIKE '%' + LOWER(:filter) + '%' " +
                                        "   OR data_referencia LIKE '%' + :filter + '%'",
                        nativeQuery = true)
        Page<IndicatorScore> findFiltered(@Param("filter") String filter, Pageable pageable);
