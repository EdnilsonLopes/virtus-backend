...
public interface ProductCycleRepository extends BaseRepository<ProductCycle> {
        ...
        @Query(value = "UPDATE virtus.produtos_ciclos " +
                        ...
                        "  AND id_ciclo = :cicloId", nativeQuery = true)
        int atualizarCicloNota(
                        @Param("entidadeId") Integer entidadeId,
                        @Param("cicloId") Integer cicloId);

        @Query(value = "SELECT analise FROM virtus.produtos_ciclos WHERE id_entidade = :entidadeId AND id_ciclo = :cicloId", nativeQuery = true)
        String findByCycleLevelIds(Long entidadeId, Long cicloId);
